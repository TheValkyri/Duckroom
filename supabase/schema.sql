-- ============================================================================
-- DUCKROOM SCHEMA SNAPSHOT (DERIVED - NON-AUTHORITATIVE)
-- CANONICAL SOURCE OF TRUTH: supabase/migrations/
-- Generated from the ordered migration set. Do not hand-edit.
--
-- COVERAGE BOUNDARY (honesty marker, updated 2026-08-24):
-- This snapshot reflects migrations 20260819 .. 20260824 (+ partial 20260829
-- lyrics_source column). It does NOT yet include:
--   20260825 authoritative_media_metadata
--   20260826 canonical_integrity_closure
--   20260827 fix_revision_actor_column
--   20260828 share_link_guest_and_token_hash
--   20260830 redteam_hardening (RPC actor binding, jwt-subquery policies,
--            upload_sessions artwork_detected_mime/width/height, lyrics purge)
--   20260831 replaygain
--   20260901 external_identities
--   20260902 user_preferences
--   20260903 atomic_playlist_reorder
...
-- Lyrics provenance (20260829): provider attribution for embedded lyrics.
...
-- -----------------------------------------------------------------------------
-- Derived snapshot tail: 20260824_duckroom_v2_integrity_hardening.sql
-- Migrations remain the canonical schema source of truth.
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Canonical artwork assets are keyed by their immutable master object key (AUTHORITATIVE: Non-fabricating MIME resolution).
  INSERT INTO public.artwork_assets (master_storage_key, display_256_key, display_512_key, display_1024_key, display_2048_key, mime_type)
  SELECT DISTINCT
    key, key, key, key, key,
    CASE
      WHEN lower(key) ~ '\.(jpg|jpeg)$' THEN 'image/jpeg'
      WHEN lower(key) ~ '\.png$' THEN 'image/png'
      WHEN lower(key) ~ '\.webp$' THEN 'image/webp'
      WHEN lower(key) ~ '\.avif$' THEN 'image/avif'
      WHEN lower(key) ~ '\.gif$' THEN 'image/gif'
      WHEN lower(key) ~ '\.svg$' THEN 'image/svg+xml'
